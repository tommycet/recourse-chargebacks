// record-v4.cjs — Playwright recording for Recourse demo video v3
// Records 9 scenes at 1920x1080, outputs raw WebM
// Scene durations match TTS narration clips
...
(async () => {
  ...
  const context = await browser.newContext({
    ...
    recordVideo: {
      dir: path.join(__dirname, 'raw-v4'),
  ...
  // === Scene 1: Title card (4s) — silent ===
  console.log('Scene 1: Title card (4s)...');
  ...
  // === Scenes 2-4: Landing page scroll (22.5s total) ===
  ...
  // Scene 2: Hero (6.6s)
  console.log('Scene 2: Hero (6.6s)...');
  await page.evaluate(() => window.scrollTo(0, 0));
  await page.waitForTimeout(6640);

  // Scene 3: How It Works (9.3s)
  console.log('Scene 3: How It Works (9.3s)...');
  try {
    await page.evaluate(() => {
  ...
  } catch { await page.evaluate(() => window.scrollTo(0, 700)); }
  await page.waitForTimeout(9280);

  // Scene 4: Live Proof (6.6s)
  console.log('Scene 4: Live Proof (6.6s)...');
  try {
    await page.evaluate(() => {
  ...
  } catch { await page.evaluate(() => window.scrollTo(0, 1400)); }
  await page.waitForTimeout(6640);

  // === Scene 5: Terminal — Arbiter Runner (12.6s) ===
  console.log('Scene 5: Arbiter runner terminal (12.6s)...');
  const arbiterText = fs.existsSync('/tmp/arbiter-output.txt')
    ? fs.readFileSync('/tmp/arbiter-output.txt', 'utf-8')
    : 'Arbiter output not found — using placeholder';

  const arbiterLines = arbiterText.split('\n').map(l => {
  ...
  await page.waitForTimeout(12640);

  // === Scene 6: Terminal — KeeperHub Pipeline (13.8s) ===
  console.log('Scene 6: KeeperHub pipeline terminal (13.8s)...');
  await page.setContent(`<html><body style="margin:0;background:#0a0a0a;display:flex;align-items:center;justify-content:center;height:100vh">
  ...
  await page.waitForTimeout(13840);

  // === Scene 7: Blockscout TX (12.3s) ===
  console.log('Scene 7: Blockscout transaction (12.3s)...');
  try {
    await page.goto('https://eth-sepolia.blockscout.com/tx/0x6ad71f82bfe80775b9588410dc1708f9d83b3f20e5fcb259926ccbffb056afa0', { waitUntil: 'domcontentloaded', timeout: 12000 });
    await page.waitForTimeout(12320);
  } catch(e) {
  ...
  // === Scene 8: KeeperHub Audit (7.9s) ===
  console.log('Scene 8: KeeperHub audit (7.9s)...');
  await page.setContent(`<html><body style="margin:0;background:#050505;display:flex;align-items:center;justify-content:center;height:100vh">
  ...
  await page.waitForTimeout(7920);

  // === Scene 9: Closing card (4.7s) ===
  console.log('Scene 9: Closing card (4.7s)...');
  await page.setContent(`<html><body style="margin:0;background:#050505;display:flex;align-items:center;justify-content:center;height:100vh">
    <div style="text-align:center">
  ...
  await page.waitForTimeout(4720);
