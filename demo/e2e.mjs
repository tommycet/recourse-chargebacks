/**
 * Recourse Escrow — E2E Test (Playwright) on Sepolia
 * 
 * 1. Starts HTTP server for web/demo.html
 * 2. Navigates to demo.html
 * 3. Verifies page loads
 * 4. Clicks Demo Mode (connects to Sepolia)
...
async function main() {
  console.log('═══ Recourse E2E Test (Sepolia) ═══\n');
  ...
  try {
    ...
    console.log('Step 3: Click Demo Mode (Sepolia)...');
    await page.click('text=Demo Mode');
    await sleep(5000); // Wait for Sepolia RPC connection
    ...
    if (walletText && walletText.includes('0x')) {
      console.log('  ✓ Demo connected to Sepolia\n');
    } else {
      console.log('  ⚠ Wallet text: ' + walletText + '\n');
    }

    // Step 4: Fill escrow form and create
    console.log('Step 4: Create escrow on Sepolia...');
    ...
    if (taskInput) {
      await taskInput.fill('e2e-sepolia-task-' + Date.now());
    }
    await page.screenshot({ path: join(SCREENSHOT_DIR, '04-form-filled.png') });

    await page.click('#createBtn');
    console.log('  Waiting for escrow creation (Sepolia block time ~12s)...');
    
    // Sepolia blocks take ~12s, wait up to 60s
    await sleep(60000);
    ...
    if (successEntry) {
      console.log('  ✓ Escrow created: ' + successEntry.trim().slice(0,100) + '\n');
    } else {
      console.log('  ⚠ No success entry found\n');
    }
    ...
    if (actionVisible) {
      ...
      if (confirmBtn && await confirmBtn.isVisible()) {
        ...
        console.log('  Waiting for confirmation (Sepolia block time ~12s)...');
        await sleep(60000);
        ...
        const confirmEntry = finalLogs.find(e => e.includes('confirmed') || e.includes('Confirmed') || e.includes('released'));
        if (confirmEntry) {
          console.log('  ✓ Delivery confirmed: ' + confirmEntry.trim().slice(0,100));
        } else {
          console.log('  Final logs: ' + finalLogs.slice(-5).join(' | ').slice(0,300));
        }
      } else {
